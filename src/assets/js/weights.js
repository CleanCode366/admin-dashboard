...
function handleFontSizes(btnId, featureItemId, featureStepsId, tickIconId, increase, btn) {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (FontSizeBtnState) {
      ...
      for (var i = 0; i < 4; i++) {
        StoragestepsHTML +=
          '<span class="' +
          (i < FontSizeClickState
            ? 'active step uwaw-features__step'
            : 'step uwaw-features__step') +
          '"></span>'
      }
      ...
      if (FontSizeStateBigger != 0) {
        ...
        var elements = document.querySelectorAll(
          '*:not(.uw-widget-custom-trigger, .uw-widget-custom-trigger span)'
        )
  ...
  document.getElementById(btnId).addEventListener('click', function () {
    ...
    for (var i = 0; i < 4; i++) {
      stepsHTML +=
        '<span class="' +
        (i < clickCount ? 'active step uwaw-features__step' : 'step uwaw-features__step') +
        '"></span>'
    }
...
function increaseAndReset() {
  ...
  for (var i = 0; i < 3; i++) {
    stepsHTML_ts +=
      '<span class="' +
      (i < clickCountText ? 'active step uwaw-features__step' : 'step uwaw-features__step') +
      '"></span>'
  }
  ...
  letterSpacing += 2 // Increase by 5px
  applyLetterSpacing()
}
...
function applyLetterSpacing() {
  const elements = document.querySelectorAll(
    '*:not(.uw-widget-custom-trigger, .uw-widget-custom-trigger span)'
  ) // Select all elements except buttons
  elements.forEach(function (element) {
    if (!isExcludedElement(element)) {
      ...
      var newSize_ts = parseFloat(currentSize_ts) + 2
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (SpacingState) {
    ...
    for (var i = 0; i < 3; i++) {
      stepsHTML_ts +=
        '<span class="' +
        (i < SpacingClickState ? 'active step uwaw-features__step' : 'step uwaw-features__step') +
        '"></span>'
    }
    featureSteps_ts.innerHTML = stepsHTML_ts
...
document.getElementById('btn-s12').addEventListener('click', function () {
  ...
  for (var i = 0; i < 3; i++) {
    stepsHTML_lh +=
      '<span class="' +
      (i < clickCount_lh ? 'active step uwaw-features__step' : 'step uwaw-features__step') +
      '"></span>'
  }
...
function toggleImages() {
  ...
  document.documentElement.id = document.documentElement.classList.contains('image-hide')
    ? 'imageHideBg'
    : ''
...
document.getElementById('dark-btn').addEventListener('click', function () {
  ...
  // Toggle the visibility of the tickIcon
  tickIcon_ht_dark.style.display =
    tickIcon_ht_dark.style.display === 'none' ? 'inline-block' : 'none'
...
function highlightLinks() {
  ...
  links.forEach(function (link, index) {
    if (!isExcludedElement(link)) {
      ...
      HighlightLinkState =
        link.style.getPropertyValue('background') === 'black !important' &&
        link.style.getPropertyValue('color') === 'yellow !important'
    }
  })
...
document.getElementById('btn-invert').addEventListener('click', function () {
  ...
  localStorage.setItem(
    'invertFeature',
    document.documentElement.classList.contains('ux4g-bg-white')
  )
})
